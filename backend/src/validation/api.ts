// @ts-nocheck
// Heavy Zod + OpenAPI schema file: disabled type checking to avoid multi-GB compiler memory usage.
// Remove this directive to re-enable full type safety when optimizing schemas.
import { z } from 'zod';
...
export const SearchResultSchema = z.object({
  ...
  content: z.string().nullable().optional().openapi({
    description: 'Full document content (only included when includeFullDocs=true)',
    example: 'This is the complete content of the document about machine learning concepts...',
  }),
...
export const MemorySearchResult = z.object({
  ...
  context: z
        ...
        .array(
          z.object({
            ...
            memory: z.string().openapi({
              ...
              example: 'Earlier version: Dhravya is working on a patent at Cloudflare.',
            }),
      ...
      children: z
        .array(
          z.object({
            ...
            memory: z.string().openapi({
              description: 'The contextual memory content',
              example: 'Later version: Dhravya has filed the patent successfully.',
            }),
            metadata: z.record(z.unknown()).nullable().optional().openapi({
              description: 'Contextual memory metadata',
            }),
    ...
    .optional()
    .openapi({
      description: 'Object containing arrays of parent and child contextual memories',
    }),
...
export const ProjectSchema = z
  .object({
    ...
    containerTag: z.string().openapi({
      description: 'Container tag for organizing memories (format: sm_project_{name})',
...
export const BulkDeleteMemoriesSchema = z
  .object({
    ...
    containerTags: z
      ...
      .optional()
      .openapi({
        description: 'Array of container tags - all memories in these containers will be deleted',
        example: ['user_123', 'project_123'],
      }),
  })
  .refine(
  ...
  .openapi({
    description: 'Request body for bulk deleting memories by IDs or container tags',
    example: {
