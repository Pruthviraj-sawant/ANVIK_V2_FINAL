...
import { Dropzone, DropzoneContent, DropzoneEmptyState } from '@ui/components/shadcn-io/dropzone';
...
const TextEditor = lazy(() => import('./text-editor').then((mod) => ({ default: mod.TextEditor })));
  ...
  const addContentMutation = useMutation({
    mutationFn: async ({
      ...
      const processingPromise = (async () => {
        ...
        if (response.error) {
          throw new Error(response.error?.message || `Failed to add ${contentType}`);
        }
        ...
        const pollForCompletion = async (): Promise<any> => {
          ...
          while (attempts < maxAttempts) {
            try {
              ...
              if (memory.error) {
                throw new Error(memory.error?.message || 'Failed to fetch memory status');
              }
    ...
    onMutate: async ({ content, project, contentType }) => {
      ...
      const previousMemories = queryClient.getQueryData(['documents-with-memories', project]);
      ...
      const optimisticMemory = {
        ...
        title: contentType === 'link' ? 'Processing...' : content.substring(0, 100),
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end w-full gap-4 mt-auto">
                    <div className="flex flex-col sm:flex-row sm:items-end gap-4 order-2 sm:order-1">
                      ...
                      <MemoryUsageRing memoriesLimit={memoriesLimit} memoriesUsed={memoriesUsed} />
                    </div>
                ...
                >
                  <div className="grid gap-4">
                    ...
                    <motion.div
                      ...
                      transition={{ delay: 0.1 }}
                    >
                      <label className="text-sm font-medium" htmlFor="link-content">
                        Link
                      </label>
                      <addContentForm.Field
                        name="content"
                ...
                >
                  <div className="grid gap-4">
                    ...
                    <motion.div
                      ...
                      transition={{ delay: 0.15 }}
                    >
                      <label className="text-sm font-medium" htmlFor="file-title">
                        Title (optional)
                      </label>
                    ...
                    >
                      <label className="text-sm font-medium" htmlFor="file-description">
            ...
            <motion.div animate={{ opacity: 1, scale: 1 }} initial={{ opacity: 0, scale: 0.95 }}>
              ...
              <div className="grid gap-4 py-4">
                ...
                >
                  ...
                  />
                  <p className="text-xs text-white/50">This will help you organize your memories</p>
                </motion.div>
              </div>
...
export function AddMemoryExpandedView() {
  ...
  const [selectedTab, setSelectedTab] = useState<'note' | 'link' | 'file' | 'connect'>('note');
  ...
  return (
    <>
      <motion.div
        ...
        initial={{ opacity: 0, y: 10 }}
      >
        <p className="text-sm text-white/70">Save any webpage, article, or file to your memory</p>
      ...
      {showDialog && (
        <AddMemoryView initialTab={selectedTab} onClose={() => setShowDialog(false)} />
      )}
    </>
  );
}
