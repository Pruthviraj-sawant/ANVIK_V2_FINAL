...
import type { ConnectionResponseSchema } from '@/validation/api';
...
export function ConnectionsTabContent() {
  ...
  useEffect(() => {
    if (!autumn.isLoading) {
      setIsProUser(
        autumn.customer?.products.some((product) => product.id === 'consumer_pro') ?? false,
      );
    }
  ...
  return (
    <div className="space-y-4">
      <div className="mb-4">
        ...
        {!isProUser && !autumn.isLoading && (
          <p className="text-xs text-foreground/50 mt-1">Connections require a Pro subscription</p>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="text-xs text-foreground/40">Choose a service below to connect</p>
        </motion.div>
      ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {connection.email && (
                      <p className="text-sm text-foreground/60">{connection.email}</p>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-sm text-foreground/60 mt-0.5">{config.description}</div>
                  </div>
                </Button>
              </motion.div>
            );
